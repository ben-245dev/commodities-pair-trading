...
class TradingStrategy:
    ...
    def split_data(self, train_pct: float = 0.2):
        """
        FIX #1: Split data into train/test to avoid look-ahead bias.
    ...
    def find_cointegrated_pairs(self, data: pd.DataFrame, corr_threshold: float = 0.85, pvalue_threshold: float = 0.05):
        """
        FIX #2: Scan cointegration ONLY on training data.
        ...
        for i in range(n):
            for j in range(i + 1, n):
                ...
                if adf_result[1] < pvalue_threshold:
                    pairs.append((s1, s2))
    ...
    def run_monte_carlo(self, returns_series: pd.Series, simulations: int = 50):
        """
        FIX #3: Monte Carlo now properly aligned and uses only realized returns.
    ...
    def backtest_pair(self, s1, s2, data: pd.DataFrame, params: dict):
        """
        FIX #4: Improved position management and cost calculation.
        Now properly tracks actual trades (not just position changes).
        NEW: Implements cooldown period after stop loss hits.
        """
        # 1. Prepare Data
        ...
        # FIX: Add 2-day lag to ensure no look-ahead
        model = RollingOLS(y, x, window=params['beta_window'])
        rolling_res = model.fit()
        res_params = rolling_res.params.shift(2)  # FIX: 2-day lag instead of 1
        ...
        # 7. FIX: Correct Cost Calculation
        ...
        # FIX: Use abs(positions) instead of abs(pos_delayed) to charge on both entry and exit
    ...
    def backtest_portfolio(self, pairs: List[Tuple[str, str]], data: pd.DataFrame, params: dict):
        ...
        for s1, s2 in pairs:
            ...
            if pnl.std() != 0:
                ...
                pair_data[name] = {
                    ...
                    'pnl': pnl, 
                    'trades': num_trades,
                    'stop_losses': sl_count  # NEW: Track stop loss events
                }
        ...
        # FIX: Improved Risk Parity with configurable window
    ...
    def run(self):
        try:
            ...
            if self.use_walk_forward:
                ...
                # FIX: Chain cumulative returns properly instead of concatenating
            ...
            # FIX: Calculate avg_trade_ret properly
            # The portfolio return divided by total trades is misleading
            # because costs are applied at the pair level, not portfolio level
            # 
            # Better metric: Average return per pair-trade
            # Or: Show cost impact explicitly
            ...
            if total_trades > 0:
                ...
                # This shows: if we had X trades, we paid Y% in total costs
                avg_cost_per_trade = total_costs_paid / total_trades
                
                # The "avg return per trade" is conceptually:
                # How much return did we generate per trading event?
                # Note: This is a portfolio-level metric and will be small
            ...
            extra_metrics = {
                ...
                'avg_cost_per_trade': avg_cost_per_trade,  # NEW: Show expected cost
...
class OptimizerWorker(QThread):
    ...
    def calculate_metric(self, port_ret):
        ...
        if self.metric_type == "Sharpe Ratio":
            ...
            return -dd.min()  # Negative because we want to minimize
            
        return -999.0

    def run(self):
        # FIX: Optimize ONLY on training data
        self.strategy.split_data(train_pct=0.2)
...
class MainWindow(QMainWindow):
    ...
    def display_results(self, cum_ret, pair_data, port_ret, mc_sims, extra_metrics, pairs):
        ...
        for i, (name, data) in enumerate(sorted_pairs):
            ...
            # NEW: Display stop losses for this pair
